...
# Main: layer slider (applies globally)
st.subheader("Layer-based Slice Control")
min_layer, max_layer = (unique_layers[0], unique_layers[-1]) if unique_layers else (0, 0)
layer_slice = st.slider(
    "Show layers up to:",
...
if xyz_axes:
    fig_xyz = px.line(
        df_slice, x="Time Step", y=xyz_axes,
...
with col1:
    ...
    if abc_axes:
        fig_abc = px.line(
            df_slice, x="Time Step", y=abc_axes,
