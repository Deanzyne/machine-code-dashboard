...
# Remove slider from sidebar; will go under 3D viewer
# Layer-range ticks for reference
def layer_ticks(min_l, max_l):
    ticks = {f"{i*10}%": int(min_l + (max_l-min_l)*i/10) for i in range(11)}
    return ticks
...
with st.expander("🌐 3D Toolpath Visualizer (Full Width)", expanded=True):
    mode_type = st.selectbox("Graph Type:", ['Line','Scatter','Streamtube'])
    mode_color = st.selectbox("Visualization Mode:", ['Layer','Extrusion Rate','Distance','Layer Time'])
    show_seams = st.checkbox("Show Layer Seams", value=False)
    show_extrema = st.checkbox("Show Layer High/Low", value=False)
    show_startstop = st.checkbox("Show Part Start/Stop", value=False)
    ...
    # Color mapping
    if mode_color=='Layer': color = df3['Layer']
    elif mode_color=='Extrusion Rate': color = df3['E'].diff().fillna(0)
    ...
    else:  # Layer Time: use average step index per layer
        layer_time = df3.groupby('Layer')['Time Step'].transform('mean')
        color = layer_time

    # Build figure
    if mode_type=='Line':
        trace = go.Scatter3d(x=df3['X'],y=df3['Y'],z=df3['Z'],mode='lines',line=dict(color=color,colorscale='Viridis',width=6))
    elif mode_type=='Scatter':
        trace = go.Scatter3d(x=df3['X'],y=df3['Y'],z=df3['Z'],mode='markers',marker=dict(color=color,colorscale='Viridis',size=4,opacity=0.6))
    else:
        trace = go.Streamtube(x=df3['X'],y=df3['Y'],z=df3['Z'],value=color,colorscale='Viridis',sizeref=0.5)

    fig3d = go.Figure(trace)
    # Add seams
    if show_seams:
        for lyr,grp in df3.groupby('Layer'):
            start, end = grp.iloc[0], grp.iloc[-1]
            fig3d.add_trace(go.Scatter3d(x=[start.X,end.X],y=[start.Y,end.Y],z=[start.Z,end.Z],mode='lines',line=dict(color='white',width=2)))
    # Add extrema
    if show_extrema:
        for lyr,grp in df3.groupby('Layer'):
            hi=grp.loc[grp['Z'].idxmax()]
            lo=grp.loc[grp['Z'].idxmin()]
            fig3d.add_trace(go.Scatter3d(x=[hi.X],y=[hi.Y],z=[hi.Z],mode='markers',marker=dict(color='yellow',size=4)))
            fig3d.add_trace(go.Scatter3d(x=[lo.X],y=[lo.Y],z=[lo.Z],mode='markers',marker=dict(color='orange',size=4)))
    # Add start/stop
    if show_startstop and not df3.empty:
        s, e = df3.iloc[0], df3.iloc[-1]
        fig3d.add_trace(go.Scatter3d(x=[s.X],y=[s.Y],z=[s.Z],mode='markers',marker=dict(color='green',size=6)))
        fig3d.add_trace(go.Scatter3d(x=[e.X],y=[e.Y],z=[e.Z],mode='markers',marker=dict(color='red',size=6)))
...
with st.expander("📈 XYZ Axes Over Time",expanded=True):
    ...
    if xyz_axes:
        ...
        if mode_xyz=='Raw': fig_xyz=px.line(dfx,x='Time Step',y=xyz_axes,template=template)
        else:
...
with st.expander("📈 ABC Axes Over Time",expanded=True):
    ...
    if abc_axes:
        if mode_abc=='Raw': fig_abc=px.line(dfx,x='Time Step',y=abc_axes,template=template)
        else:
