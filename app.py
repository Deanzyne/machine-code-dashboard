...
corners = [
    [bx[i], by[j], bz[k]] for i in range(2) for j in range(2) for k in range(2)
]
...
for e in edges:
    ...
    fig_bb.add_trace(go.Scatter3d(x=[x0,x1], y=[y0,y1], z=[z0,z1], mode='lines', line=dict(color='lightgray', width=2)))
fig_bb.update_layout(scene=dict(aspectmode='data'), template='plotly_dark', height=250, margin=dict(l=0, r=0, b=0, t=0))
